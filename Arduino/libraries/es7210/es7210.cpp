...
{
    uint8_t error;
    Wire.beginTransmission(handle->i2c_addr);    // Start the I2C transmission to the device
    // printf("AAAAAAAAAAAAAAAAAAA:%x\r\n",handle->i2c_addr);
    Wire.write(reg_addr);    // Write the register address
    Wire.write(reg_val);    // Write the register value
    error = Wire.endTransmission(true);    // End the transmission and check for errors
    if (error != 0) {    // If there's an error, print it and return the corresponding error code
        printf("ES7210 endTransmission failed with error code: %u\n", error);
        return ESP_FAIL;  // Use ESP_FAIL to indicate failure in I2C communication
    }
...
esp_err_t es7210_new_codec(const es7210_i2c_config_t *i2c_conf, es7210_dev_handle_t *handle_out)
{
    ...
    struct es7210_dev_t *handle =  (es7210_dev_t*)calloc(1, sizeof(struct es7210_dev_t));
...
esp_err_t es7210_reset(es7210_dev_handle_t handle) {
    ...
    // 发送复位命令
    ES7210_WRITE_REG(ES7210_RESET_REG00, 0xFF);  // 发送复位命令
    vTaskDelay(pdMS_TO_TICKS(10));  // 等待 10ms，让设备完成复位过程

    // 恢复为正常工作状态
    ES7210_WRITE_REG(ES7210_RESET_REG00, 0x32);  // 设置恢复工作状态
