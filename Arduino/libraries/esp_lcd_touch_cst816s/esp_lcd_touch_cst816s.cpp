...
esp_err_t i2c_read_bytes(esp_lcd_touch_handle_t tp, uint16_t reg, uint8_t *data, uint8_t len);
esp_err_t i2c_write_bytes(esp_lcd_touch_handle_t tp, uint16_t reg, uint8_t* data, uint8_t len);
...
esp_err_t esp_lcd_touch_new_i2c_cst816s(TwoWire *i2c, const esp_lcd_touch_config_t *config, esp_lcd_touch_handle_t *tp)
{
...
static void AutoSleep(esp_lcd_touch_handle_t tp, bool Sleep_State) {
    ...
    i2c_write_bytes(tp, (uint16_t)0xFE, &Sleep_State_Set, 1);
}
...
esp_err_t read_id(esp_lcd_touch_handle_t tp)
{
    uint8_t id;
...
esp_err_t i2c_read_bytes(esp_lcd_touch_handle_t tp, uint16_t reg, uint8_t *data, uint8_t len)
{
  ...
  TwoWire *i2c = (TwoWire *)(tp->config.user_data);
  i2c->beginTransmission(ESP_LCD_TOUCH_IO_I2C_CST816S_ADDRESS);
  i2c->write(reg); 
  if (i2c->endTransmission(true)){
    return ESP_FAIL;
  }
  i2c->requestFrom(ESP_LCD_TOUCH_IO_I2C_CST816S_ADDRESS, len);
  for (int i = 0; i < len; i++) {
    *data++ = i2c->read();
  }
  return ESP_OK;
